...
// Private key for tests (from test mnemonic)
import { PrivateKey } from '../types';

export const TEST_PRIVATE_KEY = Buffer.from('ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80', 'hex') as unknown as PrivateKey;

// Helper function to create PrivateKey from hex string or buffer
export function createPrivateKey(keyData: string | Buffer | Uint8Array): PrivateKey {
  if (typeof keyData === 'string') {
    return Buffer.from(keyData.replace('0x', ''), 'hex') as unknown as PrivateKey;
  } else if (keyData instanceof Buffer) {
    return keyData as unknown as PrivateKey;
  } else {
    return new Uint8Array(keyData) as unknown as PrivateKey;
  }
}