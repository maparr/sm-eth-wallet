import { WalletError, ProviderErrorCode } from './types.js';
...
export function mapProviderError(error: any, provider: string): WalletError {
  const code = error.code || error.error?.code;
  
  switch (code) {
    case -32000:
      return new WalletError(
        'Invalid transaction: ' + (error.message || 'Unknown error'),
        'INVALID_TRANSACTION'
      );
...
export function formatErrorForDisplay(error: WalletError): string {
