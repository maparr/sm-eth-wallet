...
export class EIP155Signer {
  ...
  private validateTransaction(tx: EIP155Transaction): void {
    ...
    if (tx.gasLimit > 30000000n) {
      throw new WalletError(
        'Gas limit too high (maximum 30000000)',
        'INVALID_GAS_LIMIT',
        'gasLimit'
      );
    }
  ...
  private toRLPValue(value: bigint): string {
    if (value === 0n) {
      return '0x';
    }
